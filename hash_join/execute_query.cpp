...
vector<vector<pair<string, string>>>  execute_query::Generate_Enc_query(vector<pair<string, string>>tab,unordered_map<string, vector<string>>table_name_map,vector<string>&Enc_query,string query)
{
  int len=tab.size();
...
vector<string>  execute_query::Get_hash_name(vector<pair<string, string>> re)
{
  int len=re.size();
...
//暂时只支持俩表join
unordered_map<int,vector<string>>  execute_query::get_Aes_val(string table)
{
...
vector<vector<string>> execute_query::Hash_join(vector<string> h_table)
{
